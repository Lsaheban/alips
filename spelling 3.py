...
sent=":"
def tor(x):		
		global avl_w,tel,result,ty,sol,pul, w_len,len_in,lez
		len_in,lez,w_len=len(x),list(x),len(lis)
		avl_w,tel,result,ty,sol,pul= ([] for i in range(6))
		
		for it in range(0,w_len):
			kup=list(lis[it])
...
def mon(kip,p_arg):
		for it in range(0,w_len):
			...
			if lol>0:
				...
				avl_w.append(0)
		don(p_arg,len_in,avl_w)
			
def don(x,y,z):
    for l in range(0,w_len):
        jen=len(lis[l])
        my=x[l]+.02
        sk=(z[l]/y)+.02#done
        if y < jen :
...
def race(min) :
    ...
    for j in tel:
      zul=zul+1
      if j>(.95-min):
        print("probability:",round((.95-min)*100),'%')
        result.append(lis[zul])
    if result == ty:
    	min=min+.01
    	if min>.3:
    		print('sorry did not find any match')
    		inp()
    	else:
    		race(min)
    else:
      if work==0:
          #print('you might searching :',result)
          if len(result) > 1:
              sent='/'.join(result)
          else:
              sent=' '.join(result)
...
def inp():
    ...
    if rec in lis:
      ...
      print("[warning]: please enter a valid text")
      inp()  
    else:
      work=0
      tor(rec)
def start():
    call=input("Model: large/small: ")
    if call=='large':
        active("large")
    else:
        inp()
